    ...
    public function handle()
    {
        ...
        foreach (ExpensesContact::get() as $ExpensesContact) {
            if ($ExpensesContact->expensesCategory) {
                if (ExpensesCategories::where('name', preg_replace('/[\x00-\x1F\x7F]/', '',  preg_replace('/\s+/', ' ', str_replace("'", "", str_replace('"', '', $ExpensesContact->expensesCategory->description)))))->count() == 0) {
                    ExpensesCategories::create([
                        ...
                        'tenant_id' => 1,
                        'u_id' => (ExpensesCategories::count() + 1),
                    ]);
                }
            } else {
            ...
            ExpensesAccounts::create([
                ...
                'assigned_user_id' => $ExpensesContact->Document ? $ExpensesContact->Document->user_id??1 : 1,
                'tenant_id' => 1,
                ...
                'u_id' => (ExpensesAccounts::count() + 1),
                'account_number' => preg_replace('/[\x00-\x1F\x7F]/', '',  preg_replace('/\s+/', ' ', str_replace("'", "", str_replace('"', '', $ExpensesContact->account_number)))),
                'created_at' => $ExpensesContact->Document ? ($ExpensesContact->Document->create_time) : null,
                'updated_at' => $ExpensesContact->Document ? ($ExpensesContact->Document->last_edit_time) : null,
            ]);
        ...
        foreach (ExpensesVoucher::get() as $ExpensesVoucher) {
            if ($ExpensesVoucher->supplierInvoiceStatusOption) {
                if (PurchaseOrderInvoiceStatuses::where('name', preg_replace('/[\x00-\x1F\x7F]/', '',  preg_replace('/\s+/', ' ', str_replace("'", "", str_replace('"', '', $ExpensesVoucher->supplierInvoiceStatusOption->description)))))->count() == 0) {
                    PurchaseOrderInvoiceStatuses::create([
                        ...
                        'tenant_id' => 1,
                        'u_id' => (PurchaseOrderInvoiceStatuses::count() + 1),
                    ]);
                }
            } else {
            ...
            ExpensesVouchers::create([
                ...
                'assigned_user_id' => $ExpensesVoucher->Document ? $ExpensesVoucher->Document->user_id??1 : 1,
                'currency_id' => $currency,
                ...
                'tenant_id' => 1,
                'u_id' => (ExpensesVouchers::count() + 1),
                'type' => $ExpensesVoucher->is_cash_expenses == 1 ? 'Credit' : 'Debit',
                'created_at' => $ExpensesVoucher->Document ? ($ExpensesVoucher->Document->create_time) : null,
                'updated_at' => $ExpensesVoucher->Document ? ($ExpensesVoucher->Document->last_edit_time) : null,
            ]);
        ...
        foreach (ExpensesPaymentVoucher::get() as $ExpensesPaymentVoucher) {
            ...
            ExpensesPaymentVouchers::create([
                ...
                'assigned_user_id' => $ExpensesPaymentVoucher->Document ? $ExpensesPaymentVoucher->Document->user_id??1??1 : 1,
                ...
                'tenant_id' => 1,
                'u_id' => (ExpensesPaymentVouchers::count() + 1),
                'type' => $ExpensesPaymentVoucher->expensesInvoice ? ($ExpensesPaymentVoucher->expensesInvoice->is_cash_expenses == 1 ? 'Credit' : 'Debit') : 'Debit',
                'created_at' => $ExpensesPaymentVoucher->Document ? ($ExpensesPaymentVoucher->Document->create_time) : null,
                'updated_at' => $ExpensesPaymentVoucher->Document ? ($ExpensesPaymentVoucher->Document->last_edit_time) : null,
            ]);
